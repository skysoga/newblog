<template>
	<label>
		<input class="no-margin" type="text" v-model="date" placeholder="Select date..." id="calendar">
	</label>
</template>
<script>
  import { flatpicker } from '../../utils/filters'
  export default {
    data () {
      return {
        date: ""
    ...
    mounted () {
      ...
      const fp = flatpicker(el, this.props)
    },
    watch: {
...
.flatpickr-time .numInputWrapper span.arrowUp:after {
  border-bottom-color: rgba(3,3,3,0.95);
}
.flatpickr-time .numInputWrapper span.arrowDown:after {
  border-top-color: rgba(3,3,3,0.95);
}
